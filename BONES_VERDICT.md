...
**Addendum — O'Brien, 2026-09-19. Annotated, not Bones, and it does not alter the verdict above.**
There is a further reason this screen's verdict cannot be read as a live judgement of it. `GET /api/marketplace/offerings` returns **500 in production** — measured 2026-09-19, identical under the privileged owner connection *and* the app role, so it is a Postgres `42601` syntax error from a malformed Drizzle join rather than anything to do with RLS or ORDER 011: [`api/marketplace/[...path].ts:100`](resilientsa-app/api/marketplace/[...path].ts:100) writes the join predicate into the *table* argument and passes an empty `sql` template as the join condition, emitting a stray `on` before `where`. It sits behind `if (offeringIds.length > 0)`, so an empty database returns `[]` at 200 and hides it entirely — it fails from the first offering onward.

**The Marketplace browse has therefore never had a working data path, and cannot have been judged against one by any reviewer at any point.** The `CONDITIONAL PASS` should be read as conditional on a data path that has apparently never functioned, not as a pass over a working screen whose presentation fell short. Filed for its own order as [`CREW-ORDERS/CREW-ORDER-012-DRAFT-marketplace-browse-500.md`](CREW_ORDERS/CREW-ORDER-012-DRAFT-marketplace-browse-500.md:1). While it is open, the ORDER 011 §4.2 gate carries a **named, self-retiring exemption** for it so it cannot block that order — and so that closing it forces the exemption's removal rather than letting it outlive the bug.
