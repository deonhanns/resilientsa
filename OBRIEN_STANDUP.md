...
### 2026-09-15 — LIVE INCIDENT: ORDER 011 §4.2 rollout — app cannot run under the new role, one route leaks the OTP, and the app does not read `DATABASE_URL`

**Context:** Captain reported a live incident: switching Production's `DATABASE_URL` to `resilientsa_app` broke `/steward` and `/trade` for the test account (401 on `/api/me`, "no cell" on trade), and the rollback "may not have fully taken — same symptoms still showing". Ordered to diagnose live (Playwright + terminal), fix, verify on Preview first, then Production.

**Headline: the premise was wrong, Production was never broken, and the real defect is bigger than the incident.**

1. **Production is healthy.** A real Playwright login through the UI (390×844, real OTP retrieved from Vercel runtime logs) succeeded end-to-end: `200 request-code`, `200 verify-code`, `200 GET /api/me` returning the test account's own `role`/`nodeId`/`cellId`, `/trade` rendering its real (empty) feed, `/admin` returning `200 GET /api/admin/nodes`. Not a 401 anywhere. Repeated after the fix deployed — same result.
2. **The switch and the rollback were both inert, because the app does not read `DATABASE_URL`.** `api/_lib/db.ts` is `drizzle(sql, {schema})` with `sql` from `@vercel/postgres`, whose pool resolves **`POSTGRES_URL`**. `@vercel/postgres@0.10.0` contains **zero** references to `DATABASE_URL` in any runtime `.js`/`.cjs`. So the Captain edited the one variable the API layer ignores — which is precisely why the rollback "didn't take": there was nothing for it to take. Production still connects as `neondb_owner` (`rolbypassrls=true`), so **CRIT-001 remains open, unchanged.**
3. **Where the new role *is* live — Preview — the application is completely non-functional.** Every authenticated route 500s, **including `POST /api/auth/request-code`**, i.e. login itself. No token → every downstream call 401s → `/trade` shows "no cell". That is the Captain's symptom sequence exactly, and it came from a **Preview** deployment, not Production.
4. **Why:** the runtime log gives an unambiguous cause — `cause: error: unrecognized configuration parameter "app.current_node_id"`, `code 42704`, thrown at `getSession` (`api/_lib/session.ts:20`) via `api/me.ts:18`. `42704` can *only* be raised while a policy is being evaluated, and a `BYPASSRLS` role never evaluates policies — so that error is positive proof the preview was running as the new non-bypassing role. All 21 GUC-referencing policies call `current_setting('app.current_node_id')` **without the `missing_ok` argument**, so an absent context raises instead of returning no rows. Correct form: `current_setting('app.current_node_id', true)::uuid`.
5. **§4.2's stated gate cannot catch this.** `scripts/verify-rls.ts` connects directly and sets the GUC inside one transaction — it exercises the *database with context supplied*, never the application. It would report PASS while the running app cannot authenticate anyone. Necessary, not sufficient.
6. **Two further schema defects found live:** 5 tables have RLS **enabled with zero policies** (`gifts_profiles`, `grounders`, `matches`, `programme_offerings`, `trade_completions`) → deny-all for a non-owner role; and `session_tokens_user_isolation` is `USING (user_id IN (SELECT users.id FROM users))` — a subquery on `users`, so *every* session lookup evaluates the `users` policy. Making policies NULL-tolerant alone would therefore convert a 500 into a silent "0 rows" → **login fails either way.** Node-scoped RLS on `users` cannot coexist with pre-auth session lookup as designed. That is a design decision, not a bug fix — Rule #3, Spock's.

**What I found and fixed (the one thing in my authority):**
- **CRIT-002 — live OTP leak to an unauthenticated caller.** `POST /api/auth/request-code` returned `err.message`, which Drizzle populates with the SQL **and its bound parameters** — one of which is the plaintext OTP — plus a stack trace and two env-var booleans. Reproduced live: `params: 0dd80b84…3bca,691561,…`, where `691561` is the OTP. Fixed in [`api/auth/[...path].ts`](resilientsa-app/api/auth/[...path].ts:67): now returns `{"error":"Internal server error"}` and logs the SQL prefix server-side with the `params:` tail stripped (same rationale as `OTP_DEBUG_LOG` being opt-in — an OTP in a log is a live credential). **A/B verified live:** old code leaked, fixed code clean, healthy production path still `200`. No schema change, no dependency change.

**What's now complete and where it lives:**
- [`WORF_ALERTS/2026-09-15-live-incident-order011-section42-app-role-breakage.md`](WORF_ALERTS/2026-09-15-live-incident-order011-section42-app-role-breakage.md:1) — full evidence: the per-deployment `/api/me` matrix, the route matrix, the runtime-log error, the leak reproduction, the package-level `POSTGRES_URL` proof, and the live database posture (roles, grants, 27 tables, `force_rls=0`, the 5 unpoliced tables).
- [`ENGINEERING_ESCALATIONS/2026-09-15-order011-section42-app-role-unviable.md`](ENGINEERING_ESCALATIONS/2026-09-15-order011-section42-app-role-unviable.md:1) — why §4.2 as written cannot work, three candidate resolutions, and the exact Preview remediation I did **not** apply unilaterally.
- Fix deployed to Production: commit [`38dc4d5`](resilientsa-app/api/auth/[...path].ts:67), deployment `resilientsa-gi5ekckd4`, created 2026-09-15 01:48:59 SAST. Verified: authenticated `/api/me` **200**, `npm run build` zero errors (77 modules), **smoke test 15/15 exit 0**.
- Preview-verified *before* production, per the pattern: branch `fix/auth-error-body-leak` → preview `fe6zvgxn7` returned the clean body while the 20:09 preview returned the leaking one. Used the new-role preview as a deliberate fault injector for the exact 500 path being fixed.

**Corrections I have to make against my own record — two, both mine:**
1. **I misread `vercel ls`.** Its default page is **not** date-ordered, and I briefly concluded "no deployment in 3 days". Wrong: `vercel ls --environment production` shows three Production deployments on 2026-09-14 (20:08, 20:17, 20:18) plus two Previews at 20:09. I corrected it within the same session, but the first reading was wrong and anyone reading only that line would have been misled. **`--environment` is required for a trustworthy ordering.**
2. **I stated the env-var "created 15d ago" timestamp as evidence the var had not been changed.** That column is creation, not last-update, so it proves nothing about an edit. The conclusion it was supporting (the edit being inert) is independently and properly proven by the package-level `POSTGRES_URL` finding and by Production returning 200 — but the timestamp argument was not sound and I withdrew it.

**What's blocked, and on whom:**
- **Spock — ORDER 011 §4.2, blocked.** Cannot be promoted to Production as written: doing so makes login impossible platform-wide. Three schema/architecture decisions needed (escalation file above). Nothing changed on the strength of it (Rule #3).
- **Captain — one authorisation needed:** reverting Preview's `POSTGRES_URL` to the `neondb_owner` string, so Preview stops 500ing. I did **not** do it: the value is a Vercel **Sensitive** var (unreadable — `vercel env pull` returns `[SENSITIVE]`), so I cannot read the current value or verify that what I would write is what Preview held before, and reverting would reverse a step Spock authored. Exact commands are in the escalation.
- **Also recorded, deliberately not fixed (no scope creep, no order):** `GET /api/listings?cell_id=…` (depth 0) and `GET /api/marketplace/offerings/mine` (depth 2) are unroutable by construction per ORDER 010's own rule 2 — observed `404` on Production. So the Trade Exchange feed can never show real listings, and the grounder lookups cannot be live-tested at all. Needs its own order.
- **`AGENTS.md` Critical Rule #2 names `DATABASE_URL`** as the app's connection variable. It is not. Flagged for Spock rather than edited — it is crew doctrine and it is wrong in a way that caused this incident.

**Protocol/pattern checked against:**
- `AGENTS.md` #1 (build + api typecheck before every push — build clean, only the pre-existing `TS5107`), #2/#5 (no secrets: `.env.local` read only via dotenv at runtime, never printed, never staged; the phone number, the OTP and the session token were kept in shell variables only, never written to any tracked file; all temp files containing them deleted at session end), #3 (**no schema change — all three blockers deliberately left to Spock**), #4 (no project dependency added; Playwright installed under `/tmp/resa-pw`, `git status` clean), #10 (this entry).
- `SCOTTY_PATTERNS.md` **Pattern 003** — the leak fix uses its rule (an OTP in a log is a live credential) and extends it: not just logs, but error *responses*. **Pattern 005** — why the failing query surfaced only at request time, not at import.
- ORDER 011 §3/§4.1/§4.2/§4.3 and `CREW-ORDERS/CREW-ORDER-011-section-4.2-execution.md` (Spock, 2026-09-13) read in full before touching anything.

**Anything flagged to Worf or Bones:**
- **Worf — CRIT-002 is a live credential exposure that existed whenever the insert path failed** (not only under the new role), and it was found by following a 500 body rather than trusting a status code. Two related hardening items in the same file: the response previously disclosed the presence of `DATABASE_URL`/`ENCRYPTION_KEY`, and `verify-rls.ts` prefers the variable the app does not use, which is a false-confidence hazard for whoever runs it next.
- **Bones — none owed for this session.** No human-facing UI was changed; the fix is an error-body change on a 500 path.
- **Disclosure, per the 2026-09-11 precedent:** two 6-digit **test OTPs appeared in my own session output**, because printing the leaking response body verbatim is how the leak was proven. Both were for a **fabricated probe number that belongs to no person**, and both had expired (10-minute TTL) hours before this entry was written — so, unlike the 2026-09-11 token, nothing needs re-issuing. The values are deliberately not reproduced here. The real test account's phone number, OTP and session token were never printed, never written to any file, and are not in any commit.

**Next:** (1) **Spock: §4.2 redesign** (recommend a split connection identity for auth, plus NULL-tolerant policies regardless) — ORDER 011 milestones 3–4 and closure are blocked on it. (2) **Captain: authorise the Preview `POSTGRES_URL` revert.** (3) `AGENTS.md` Rule #2 correction. (4) New order for the two unroutable client routes.
